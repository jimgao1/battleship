...
public class HuntAlgorithm {
	...
	public static int[][] huntProb = new int[10][10];
		...
		for (int cX = 0; cX < AlgorithmMain.gridSizeX; cX++) {
			for (int cY = 0; cY < AlgorithmMain.gridSizeY; cY++) {
				...
				for (int p = 0; p < AlgorithmMain.shipCount; p++) {
					int lX = cX + AlgorithmMain.shipSize[p][0] - 1, lY = cY
							+ AlgorithmMain.shipSize[p][1] - 1;

					if (lX >= AlgorithmMain.gridSizeX
							|| lY >= AlgorithmMain.gridSizeY)
						continue;
				...
				for (int p = 0; p < AlgorithmMain.shipCount; p++) {
					int lX = cX + AlgorithmMain.shipSize[p][1] - 1, lY = cY
							+ AlgorithmMain.shipSize[p][0] - 1;
		...
		for (int i = 0; i < AlgorithmMain.gridSizeY; i++) {
			for (int j = 0; j < AlgorithmMain.gridSizeX; j++)
				System.out.printf("%d[%d]\t",
						(int) AlgorithmMain.gridProbability[j][i],
						AlgorithmMain.gridState[j][i]);
			System.out.println();
		}
